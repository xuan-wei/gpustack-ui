export default {
  ...
  'models.form.rules.name':
    '长度不超过 63 个字符，只能包含字母、数字、点（.）、下划线（_）、连字符（-）和冒号（:），且必须以字母或数字开头和结尾。冒号会被自动替换为连字符（即 qwen2.5:14b → qwen2.5-14b）。',
  ...
  'models.form.gpuMemoryMinGib.tips':
    '为每张参与该模型的 GPU 设置最少占用的绝对显存（GiB）。设置后会反推出有效的 --gpu-memory-utilization，并与上限一起对 vLLM 调度与启动生效；留空表示不设下限。',
  'models.form.gpuMemoryMaxGib': 'GPU 显存上限（GiB）',
  'models.form.gpuMemoryMaxGib.tips':
    '为每张参与该模型的 GPU 设置最多占用的绝对显存（GiB）。设置后会反推出有效的 --gpu-memory-utilization；与下限相等即固定值；留空表示不设上限。',
