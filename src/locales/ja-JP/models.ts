export default {
  ...
  'models.form.gpuMemoryMinGib.tips':
    'このモデルが使用する各 GPU で最低限確保する絶対 VRAM (GiB)。指定すると GPUStack が --gpu-memory-utilization を逆算し、ユーザー指定値を上書きします。小数第1位まで対応（例: 13.5）。空欄は下限なし。',
  'models.form.gpuMemoryMaxGib': 'GPU メモリ上限 (GiB)',
  'models.form.gpuMemoryMaxGib.tips':
    'このモデルが使用する各 GPU で確保する最大の絶対 VRAM (GiB)。指定すると GPUStack が --gpu-memory-utilization を逆算し、ユーザー指定値を上書きします。下限と等しくすると固定値になります。小数第1位まで対応（例: 13.5）。空欄は上限なし。',
