export default {
  ...
  'models.form.gpuMemoryMinGib.tips':
    'Minimum absolute VRAM (GiB) to claim on each GPU used by this model. When set, GPUStack back-derives an effective --gpu-memory-utilization for vLLM, overriding any user-specified value. Supports one decimal (e.g. 13.5). Leave blank for no floor.',
  'models.form.gpuMemoryMaxGib': 'GPU Memory Ceiling (GiB)',
  'models.form.gpuMemoryMaxGib.tips':
    'Maximum absolute VRAM (GiB) to claim on each GPU used by this model. When set, GPUStack back-derives --gpu-memory-utilization from this on a per-GPU basis, overriding any user-specified value. Set equal to the floor for a fixed value. Supports one decimal (e.g. 13.5). Leave blank for no ceiling.',
